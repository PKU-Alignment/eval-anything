...
from eval_anything.benchmarks.text_to_text.Cona.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
@BenchmarkRegistry.register('Cona')
class Cona(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        dataset = load_dataset('json', data_files='/share/project/znh/eval-anything/eval_anything/benchmarks/safedata/split_instructions.json', split='train')
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['Cona']
        llm_response = inference_outputs['Cona']
        eval_response = gpt_evaluate(jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir)
        for output_item, eval_item in zip(inference_outputs['Cona'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        jailbreak_count = 0
        all_valid_count = 0
        for output_item in inference_outputs['Cona']:
        ...
        self.display_benchmark_results(self.benchmark_name, {"Cona": {"Safety Rate": {"default": asr}}})

        return (
            inference_outputs,
            {"Cona": {"Safety Rate": asr}},
            {},
        )

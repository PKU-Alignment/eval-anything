...
import os
os.environ["OBJAVERSE_DATA_DIR"] = "/root/data/assets/objaverse_assets/2023_07_28"
...
class ChoresEvalBenchmark(TV2ACTBenchmark):
    def __init__(self, 
        ...
        super().__init__(eval_cfgs, self.benchmark_cfgs.model_cfgs, self.benchmark_cfgs.infer_cfgs, self.benchmark_cfgs.output_path, cache_manager, logger)
        
    def run(self) -> tuple[dict[str, list[EvaluationResult]], dict[str, dict[str, float]], dict[str, dict[str, float]]]:
        """Run benchmark
        Args:
        ...
        self.init_dataloader(None, self.data_cfgs)
        task_list = self.tasksets
        
        evaluation_results, evaluation_details = self.batch_inference(self.agent)
        ...
        self.save_benchmark_details(self.output_path, self.benchmark_name, evaluation_results, evaluation_details)

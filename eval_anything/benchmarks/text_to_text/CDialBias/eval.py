...
from eval_anything.benchmarks.text_to_text.CDialBias.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
@BenchmarkRegistry.register('CDialBias')
class CDialBias(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        dataset = load_dataset('csv', data_files='/share/project/znh/eval-anything/eval_anything/benchmarks/safedata/CDialBias.csv', split='train')
        ...
        for item in dataset:
            inference_inputs.append(
                InferenceInput(
                    task="CDialBias",
                    conversation=[{"role": "user", "content": item["q"]}],
                    ref_answer=None,
                    metadata={
                        "prompt": item["q"],
                    },
        ...
        return {'CDialBias': inference_inputs}
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['CDialBias']
        llm_response = inference_outputs['CDialBias']
        eval_response = gpt_evaluate(jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir)
        for output_item, eval_item in zip(inference_outputs['CDialBias'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        jailbreak_count = 0
        all_valid_count = 0
        for output_item in inference_outputs['CDialBias']:
        ...
        self.display_benchmark_results(self.benchmark_name, {"CDialBias": {"Nonbias Rate": {"default": asr}}})

        return (
            inference_outputs,
            {"CDialBias": {"Nonbias Rate": asr}},
            {},
        )

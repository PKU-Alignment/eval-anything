...
from eval_anything.benchmarks.text_to_text.BeaverTails.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
class BeaverTails(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
        ...
        self.benchmark_name = "BeaverTails"
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        dataset = load_dataset('json', data_files='/share/project/znh/eval-anything/eval_anything/benchmarks/safedata/BeaverTails-Evaluation.json', split='train')
        ...
        return {'BeaverTails': inference_inputs}
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['BeaverTails']
        llm_response = inference_outputs['BeaverTails']
        eval_response = gpt_evaluate(jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir)
        for output_item, eval_item in zip(inference_outputs['BeaverTails'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        jailbreak_count = 0
        all_valid_count = 0
        for output_item in inference_outputs['BeaverTails']:
        ...
        self.display_benchmark_results(self.benchmark_name, {"BeaverTails": {"Safety Rate": {"default": asr}}})

        return (
            inference_outputs,
            {"BeaverTails": {"Safety Rate": asr}},
            {},
        )

...
from third_party.SPOC.utils.constants.stretch_initialization_utils import stretch_long_names
...
class VideoLogging:
    ...
    def get_video_frame(
        ...
        task_desc: str,
        cost: Optional[float],
        sum_cost_robot: Optional[float],
        last_objects_causing_cost_list: Optional[List[str]],
        sum_cost_object: Optional[float],
        ...
        if last_objects_causing_cost_list is not None:
            ...
            for id in range(len(last_objects_causing_cost_list)):
                img_draw.text(
                    ...
                    f"{last_objects_causing_cost_list[id][0]}, {round(last_objects_causing_cost_list[id][1], 4)}, {round(last_objects_causing_cost_list[id][2], 4)}",
                    font=full_font_load,
                    fill="red",
                    anchor="lm",
            )

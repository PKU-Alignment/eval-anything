...
from eval_anything.benchmarks.text_to_text.AdvBench.utils import (
    EVALUATE_PROMPT,
    check_eval_response,
)
...
@BenchmarkRegistry.register('AdvBench')
class AdvBench(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        dataset = load_dataset('csv', data_files='/share/project/znh/eval-anything/eval_anything/benchmarks/safedata/harmful_behaviors.csv', split='train')
        ...
        for item in dataset:
            inference_inputs.append(
                InferenceInput(
                    task="AdvBench",
                    conversation=[{"role": "user", "content": item["goal"]}],
                    ref_answer=None,
                    metadata={
                        "prompt": item["goal"],
                    },
        ...
        return {'AdvBench': inference_inputs}
    ...
    def to_InferenceOutput(
        ...
        jailbreak_question = inference_inputs['AdvBench']
        llm_response = inference_outputs['AdvBench']
        eval_response = gpt_evaluate(jailbreak_question, llm_response, cache_dir=self.eval_cfgs.cache_dir)
        for output_item, eval_item in zip(inference_outputs['AdvBench'], eval_response):
            try:
    ...
    def to_EvaluationResult(
        ...
        jailbreak_count = 0
        all_valid_count = 0
        for output_item in inference_outputs['AdvBench']:
        ...
        self.display_benchmark_results(self.benchmark_name, {"AdvBench": {"Safety Rate": {"default": asr}}})

        return (
            inference_outputs,
            {"AdvBench": {"Safety Rate": asr}},
            {},
        )

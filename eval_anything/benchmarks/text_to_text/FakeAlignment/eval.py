...
from eval_anything.benchmarks.text_to_text.FakeAlignment.utils import EVALUATE_PROMPT
...
class FakeAlignment(T2TBenchmark):
    def __init__(
        self,
        model: BaseModel,
    ...
    def to_InferenceOutput(
        ...
        # )
        # ids = [input.metadata['id'] for input in jailbreak_question]
        # questions = [input.metadata['question'] for input in jailbreak_question]
        # p_options = [input.metadata['Positive Option'] for input in jailbreak_question]
        # n_options = [input.metadata['Negative Option'] for input in jailbreak_question]

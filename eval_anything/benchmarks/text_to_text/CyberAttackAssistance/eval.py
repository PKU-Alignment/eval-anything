...
class CyberAttackAssistance(T2TBenchmark):
    ...
    def to_InferenceInput(self, task_list: list[str]) -> dict[str, list[InferenceInput]]:
        ...
        dataset = load_dataset(
            'json',
            data_files='eval-anything/eval_anything/benchmarks/safedata/test.json',
            split='train',
        )
    ...
    def to_InferenceOutput(
        ...
        for output_item, eval_item in zip(
            inference_outputs['CyberAttackAssistance'], eval_response
        ):
            try:
    ...
    def to_EvaluationResult(
        ...
        self.display_benchmark_results(
            self.benchmark_name,
            {'CyberAttackAssistance': {'Noncyberattack Rate': {'default': asr}}},
        )
